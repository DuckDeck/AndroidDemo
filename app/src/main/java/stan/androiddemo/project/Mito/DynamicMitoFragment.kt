...
class DynamicMitoFragment : Fragment(), SwipeRefreshLayout.OnRefreshListener {
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        mAdapter = object:BaseQuickAdapter<ImageSetInfo,BaseViewHolder>(R.layout.image_set_item,arrImageSet){
            override fun convert(helper: BaseViewHolder, item: ImageSetInfo) {
                ...
                ImageLoadBuilder.Start(this@DynamicMitoFragment.context,img,item.mainImage).setProgressBarImage(progressLoading).build()
                helper.setText(R.id.txt_image_title,item.title)
                helper.setText(R.id.txt_image_tag,item.durationStr)
                helper.setText(R.id.txt_image_resolution,item.sizeStr)
                helper.setText(R.id.txt_image_theme,item.category)
                ...
                imgCollect.setOnClickListener {
                    if (item.isCollected){
                        ...
                        if (result){
                            Toast.makeText(this@DynamicMitoFragment.context,"收藏成功", Toast.LENGTH_LONG).show()
                        }
                        else{
                            Toast.makeText(this@DynamicMitoFragment.context,"收藏失败", Toast.LENGTH_LONG).show()
                        }
                    }
                }
        ...
        loadingView = View.inflate(this@DynamicMitoFragment.context,R.layout.list_loading_hint,null)
        failView = View.inflate(this@DynamicMitoFragment.context,R.layout.list_empty_hint,null)
    ...
    private fun loadData(){
        ImageSetInfo.dynamicSets(cat,index) { v:ResultInfo ->
            activity.runOnUiThread {
                ...
                if (v.code != 0) {
                    Toast.makeText(this@DynamicMitoFragment.context,v.message, Toast.LENGTH_LONG).show()
