...
interface TokenAPI{
    ...
    abstract fun httpsGetTokenRx(@Header(Config.Authorization) authorization: String, @Field("grant_type") grant: String,
                                 @Field("username") username: String, @Field("password") password: String): Observable<TokenBean>

    //刷新token接口
    @FormUrlEncoded
    @POST("https://huaban.com/oauth/access_token/")
    abstract fun httpsRefreshTokenRx(@Header(Config.Authorization) authorization: String, @Field("grant_type") grant: String,
